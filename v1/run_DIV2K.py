...
from data import DatasetFromFolder, DatasetFromFolder_plot
...
testset = DatasetFromFolder_plot()
...
#"model_100.pth"
model = torch.load("model_100.pth").to(device)
avg_psnr = 0
for ind, batch in enumerate(testloader):
#            input, target = batch[0].to(device), batch[1].to(device)
    input,input_c,input_b, target,target_c,target_b = batch[0].to(device), batch[3].to(device),batch[6].to(device),batch[2].to(device),batch[5].to(device),batch[8].to(device)
    input = F.interpolate(input, scale_factor=4, mode='bilinear', align_corners=False)
    input_c = F.interpolate(input_c, scale_factor=4, mode='bilinear', align_corners=False)
    input_b = F.interpolate(input_b, scale_factor=4, mode='bilinear', align_corners=False)
